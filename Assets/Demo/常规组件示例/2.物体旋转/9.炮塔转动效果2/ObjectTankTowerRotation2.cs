...
public class ObjectTankTowerRotation2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!IsRotating)
        {
            ...
            StartCoroutine(RotateByUpVector(target.transform.position));
        }
    }
    ...
    /// <returns></returns>
    IEnumerator RotateByUpVector(Vector3 target)
    {
        IsRotating = true;

        Vector3 direction = (target - transform.position).normalized;

        while (Vector3.Angle(transform.up, direction) >= 0.5f)
        {
            //直接修改UP向量，让其旋转至指向
